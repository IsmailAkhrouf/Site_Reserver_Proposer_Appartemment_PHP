	...
	if(isset($_GET["Submit"])){
		if(isset($_GET["email"])  && isset($_GET["pass"]) && isset($_GET["rpass"]) ){
			if(strcmp($_GET["pass"], $_GET["rpass"]) == 0){
				...
				$stmt1 = $pdo->prepare("select Nom from admin where email=:email");
				$stmt1->execute(array(':email'=>$_GET["email"]));
				$nb=$stmt1->rowCount();
				if($nb >= 0){
					$code = rand(1000000,9999999);
					$_SESSION["code1"]=$code;
					$_SESSION['EmailAdmin']=$_GET["email"];
					$_SESSION["passeadmin"]=$_GET["pass"];
					$email->confirmation_changement_password($_SESSION['EmailAdmin'],$_SESSION["code1"],$_SESSION['NomAdmin']);
					header("Location: ConfirmerAdmin.php");
				}
				else{
				   $str='<br><div style="text-align:center">Admin nexiste pas!!!!</div>';
				}
			}
			else{
...
<body>
    <div id="logreg-forms">
        <form class="form-signin" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="get">
            <h2 class="h3 mb-3 font-weight-normal" style="text-align: center">Changement Motdepasse Admin:</h2>
            <input type="email" id="inputEmail" class="form-control" placeholder="Address-email" required="" name="email">
            <input type="password" id="inputPassword" class="form-control" placeholder="Nouvelle Motdepasse " required="" name="pass">
			<input type="password" id="inputPassword" class="form-control" placeholder="Repeter Motdepasse " required="" name="rpass">
