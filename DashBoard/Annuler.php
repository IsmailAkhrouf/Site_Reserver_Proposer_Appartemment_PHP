...
<html lang="en">
<head>
<title>Confirmer Annulation Des Reservations</title>
...
<div id="page-wrapper">
	...
	$stmt1 = $pdo->prepare
	        ("(select annulereservation.idReservation,annulereservation.idAppartement,appartement.address,reservation.ville,annulereservation.emailreser,'accepter' as etat
			from appartement,annulereservation,reservation
			where reservation.emailreser=annulereservation.emailreser
			and reservation.idReservation =annulereservation.idReservation 
			and reservation.idAppartement = appartement.idAppartement)");
								 
								
	echo '<div class="col-sm-12 col-md-12 well" id="content">';
				   echo ' <h3><b>Confirmation Des Dommandes annulation Des Reservations: </b></h3><br/>';
	$stmt1->execute();
	while($version = $stmt1->fetch()){
	if($version["etat"]=='accepter'){
		echo '<form action="Ann-Reser.php" method="get">
			 <input type="hidden" value="'.$version["idReservation"].'" name="idReservation" />
			 <input type="hidden" value="'.$version["emailreser"].'" name="emailreser" />';
		...
		Etat : <span style="color:red;">'.$version["etat"].'</span></div>
		      ...
		      <button type="submit" class="btn" name="Annuler">Confirmer</button>
		</div></form>
		';
	}
	}
