...
<div id="page-wrapper">
	...
	$stmt1 = $pdo->prepare
	        ("(select idAppartement,ville,address,Nbrchambre,Nbretage,prixnuit,appartementproposee.email,'pendu' as etat
			from appartementproposee,client
			where client.email=appartementproposee.email 
	...
	echo '<div class="col-sm-12 col-md-12 well" id="content">';
				   echo ' <h3><b> Ajouter Les Appartements Proposees:</b></h3><br/>';
	$stmt1->execute();
	while($version = $stmt1->fetch()){
	if($version["etat"]=='pendu'){
		echo '<form action="Ajou-Sub.php" method="get">
		...
		Email proprietaire: '.$version["email"].'<br>
		Etat : <span style="color:red;">'.$version["etat"].'</span></div>
		</div><button type="submit" class="btn" name="btn">Ajouter</button>
		      <button type="submit" class="btn" name="btn1">Supprimer</button>
		</div></form>
		';
	}
	}
