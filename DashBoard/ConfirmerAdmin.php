...
$nb=0;
	if(isset($_GET["submit1"])){
		if(isset($_GET["code"])){
			if(strcmp($_GET["code"],$_SESSION["code1"])==0){
				$dsn = "mysql:host=localhost:3307;dbname=locationline";
				$user = "root";
				$passwd = "";
				$pdo = new PDO($dsn, $user, $passwd);
				$stmt = $pdo->prepare("update admin set Mdp=:mdp where email=:email");
				$mdp=$_SESSION["passeadmin"];
				$email=$_SESSION['EmailAdmin'];
				$stmt->execute(array(':mdp'=>$mdp,':email'=>$email));
				$str='<br><div style="text-align:center">Vous avez changer motdepasse avec succes<br> <a href="DashBoard.php">Retour Accueil</a></div>';
...
<body>
    <div id="logreg-forms">
        <form class="form-signin" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="get">
            ...
            <p>Verifier Votre Email Avec Le Code Qui Est Envoyé Vers L'email: <?php  echo $_SESSION['EmailAdmin']; ?></p>
