...
<html lang="en">
<head>
<title>Supprimer Appartement</title>
...
<div id="page-wrapper">
	...
	$stmt1 = $pdo->prepare
	        ("(select annuleproposition.idAppartement,address,ville,annuleproposition.emailprop,'accepter' as etat
			from appartement,annuleproposition
			where appartement.emailprop=annuleproposition.emailprop
			and appartement.idAppartement =annuleproposition.idAppartement )");
								 
								
	echo '<div class="col-sm-12 col-md-12 well" id="content">';
				   echo ' <h3><b>Confirmation Des Dommandes annulation Des Appartements: </b></h3><br/>';
	$stmt1->execute();
	while($version = $stmt1->fetch()){
	if($version["etat"]=='accepter'){
		echo '<form action="Sub.php" method="get">
		...
		Email proprietaire: '.$version["emailprop"].'<br>
		Etat : <span style="color:red;">'.$version["etat"].'</span></div>
		</div>
		      <button type="submit" class="btn" name="btn">Confirmer</button>
		</div></form>
		';
	}
	}
