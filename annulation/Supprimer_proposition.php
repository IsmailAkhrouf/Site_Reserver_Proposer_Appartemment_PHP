...
try{
   ...
   $stmt1=$bdd->prepare("select * from annuleproposition where idAppartement=:id");
   $stmt1->execute(array(':id'=>$_GET['idAppartement']));
   if($stmt1->rowCount()==0){
   $stmt=$bdd->prepare("insert into annuleproposition Values (?,?)");
	   $stmt->bindParam(1,$_GET['idAppartement']);
	   $stmt->bindParam(2,$_SESSION['email']);
	   $stmt->execute();
	   $nid = uniqid('no');
	   $sujet="Demande Annulation Proposition";
	   $objet="Appartement";
	...
	$s='<div class="container-fluid">
			<div class="alert alert-success" role="alert">
				<strong>Demande Annulation Proposition Complete </strong> <a href="#" class="alert-link"> La Proposition est entree avec succes . Vous devriez recevoir un email de confirmation de votre Demande Annulation Proposition apres un jour au maximum.</a>.
	   ...
	   $s='<div class="container-fluid">
			<div class="alert alert-success" role="alert">
				<strong>Demande Annulation Proposition Incomplete !!!  </strong>  <a href="#" class="alert-link">Vous ne pouvez pas refaire la demande de annulation </a>.
				<a href="../indix.php">Retour Accueil</a>
			</div>
    </div>';
   }
