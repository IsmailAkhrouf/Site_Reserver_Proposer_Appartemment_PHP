	...
	if(isset($_GET["submit1"])){
		if(isset($_GET["code"])){
			if(strcmp($_GET["code"],$_SESSION["code"])==0){
				...
				$stmt = $pdo->prepare("update client set Motdepasse=:mdp where email=:email");
				$mdp=$_SESSION["passe"];
				$email=$_SESSION["email"];
				$stmt->execute(array(':mdp'=>$mdp,':email'=>$email));
				$str='<br><div style="text-align:center">Vous avez changer motdepasse avec succes<br> <a href="../indix.php">Retour Accueil</a></div>';
